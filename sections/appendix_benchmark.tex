...
Human records constrain the generator rather than serving as episode templates.
...
\begin{table}[H]
\caption{Audited source contributions to \pfdbench{} generation.}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Source} & $\mathbf{N_{raw}}$ & $\mathbf{N_{std}}$ & $\mathbf{N_{used}}$ & $\mathbf{N_{ref}}$ \\
\midrule
CASAS Aruba & 1,602,820 & 20,252 & 138 & 140 \\
HD-EPIC & 59,454 & 10,786 & 379 & 400 \\
ParaHome & 212 & 860 & 54 & 54 \\
HOMER+ (sim.) & 65 & 1,991 & 268 & 284 \\
\bottomrule
\end{tabular}
\end{table}

The raw unit is a sensor row for CASAS, an annotation row for HD-EPIC, and a
sequence for ParaHome and HOMER+. CASAS motion events are mapped to the benchmark
room ontology and separated into sessions when the inactivity gap exceeds
900 seconds; room occupancy, time-of-day, and weekday statistics are measured
from these sessions, whereas activity compatibility is benchmark-authored.
HD-EPIC narration verbs and nouns are normalized through canonical action and
object aliases to estimate object--action frequencies, but do not determine
HSSD destinations. ParaHome annotations and object transforms provide measured
displacement and transition-timing evidence; 741 of its 860 normalized records
contain usable motion summaries. HOMER+ is tracked separately as simulated
long-horizon evidence for activity order and terminal-state priors, not as a
human-observation source or model input. ARAS and OPPORTUNITY inform ontology
coverage and adapter design, but their adapters are not in the audited
consumption manifests; we therefore assign them neither record counts nor fitted
generator parameters in the current release.
...
quantities comprise CASAS occupancy timing, HD-EPIC action--object frequencies,
and ParaHome displacement and transition timing. Legal receptacles, reachable
