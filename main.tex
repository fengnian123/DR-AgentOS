...
outside its view. Long-lived embodied agents must therefore infer the current
state from intermittent observations and decide where to gather evidence when
that inference is uncertain. Recent memory and mapping methods retain spatial
experience, while predictive scene representations and routine models learn
temporal regularities in object states and placements. Translating these
advances into navigation requires retaining alternative hypotheses and revising
decisions as new evidence arrives. To address this challenge, we introduce
\evolvingworldnav{}, an embodied agent
that turns timestamped 3D entity histories into a belief over current object
locations. A continuous-time Transformer learns to predict this
persistence--relocation belief from irregular observations, separating whether
the last observed state remains valid from where the object may have moved.
Within a frozen vision--language control loop, the agent retains alternative
location hypotheses and balances candidate probability against inspection
cost. It then updates its belief and replans using visibility-aware evidence,
distinguishing an empty location from an inconclusive view. We also introduce
\pfdbench{}, a
...
highlighting the need to retain uncertainty and gather evidence when history
